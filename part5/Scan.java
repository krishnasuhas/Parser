...
public class Scan {
    ...
    private static final int EOF = -1;      // (int rather than char to handle EOF)
    

   
    public Scan() 
    {     
	// the constructor -- here initializes variables...
	...
	while(true) {
		...
		switch( c ) {
			...
			return ccase1('<',TK.DO);
		    case '>':
			return ccase1('>',TK.DOWHILE);
		    case '~':
			return ccase1('~',TK.SCOPING);			
		    case '=':
			return ccase1('=',TK.ASSIGN);
		    case '&':
			return ccase1('&',TK.GRTTHAN);			
		    case '$':
			return ccase1('$',TK.LESSTHAN);
		    case '?':
			return ccase1('$',TK.ENDFORBLK);
		    case '"':
			return ccase1('"',TK.FOR);
		    case '`':
			return ccase1('`',TK.ENDFOR);			
		    case ';':
			return ccase1(';',TK.INFOR);
		    case EOF:
			got_eof = true;
